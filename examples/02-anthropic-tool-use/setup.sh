#!/usr/bin/env bash
# Setup for 02-anthropic-tool-use — installs deps. No policies needed.
#
...
[[ -n "${ANTHROPIC_API_KEY:-}" ]] || { echo "error: ANTHROPIC_API_KEY empty in .env — add it and re-run" >&2; exit 1; }
...
# 3. No policies for this example — instrumentation only.
